    ...
    <div className="p-8">
      <PageHeader
        ...
        action={unreadCount > 0 && (
          <GlassButton variant="secondary" onClick={markAllRead}>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          <Bell className="w-12 h-12 text-violet-500/10 mx-auto mb-3" />
          <p className="text-white/30">No notifications yet</p>
          <p className="text-xs text-white/20 mt-1">Activity from your communities will appear here</p>
        </div>
      ) : (
        <div className="space-y-2">
          {notifications.map((notif, i) => (
            <motion.div key={notif.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
              className={`flex items-start gap-4 p-4 rounded-2xl border transition-all ${typeColor[notif.type] || 'border-white/6 bg-white/3'} ${!notif.is_read ? 'ring-1 ring-violet-500/20' : 'opacity-55'}`}>
              ...
              {!notif.is_read && (
                <button onClick={() => markRead(notif.id)} className="flex-shrink-0 p-1.5 rounded-lg text-white/30 hover:text-violet-400 hover:bg-violet-500/10 transition-all">
